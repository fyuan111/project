...
#define _FML_LOG(level_str, color, fmt, ...) \
    printf(color "[%10u][%s]" fmt FML_COLOR_RESET "", get_time(), level_str, ##__VA_ARGS__)
#elif FML_LOG_TIMESTAMP
#define _FML_LOG(level_str, color, fmt, ...) \
    printf("[%10u][%s]" fmt "", get_time(), level_str, ##__VA_ARGS__)

#elif FML_LOG_COLOR
#define _FML_LOG(level_str, color, fmt, ...) \
    printf(color "[%s]" fmt FML_COLOR_RESET "", level_str, ##__VA_ARGS__)
#else
#define _FML_LOG(level_str, color, fmt, ...) \
...
#define fml_error(fmt, ...) _FML_LOG("ERR", FML_COLOR_ERROR, fmt, ##__VA_ARGS__)
